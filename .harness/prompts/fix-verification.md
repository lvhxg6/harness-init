...
5. 如果失败是 `No test files found`、测试契约缺失，或某个 backend test script 没有对应测试文件，优先补齐该 backend 项目内的单元/域/集成测试。API/E2E specs 不能替代 backend 自身的 `npm test`、`mvn test`、`pytest` 或 `go test` 输入。
6. 如果失败来自第三方 API 适配，但当前是 mock 验证，仍要保证 live provider 的真实调用代码不退化为 stub。
7. 禁止安装依赖、联网安装浏览器或启动长驻 dev server。不要运行 `npm install`、`npm ci`、`pnpm install`、`yarn install`、`npx playwright install`、`./scripts/start-test-env.sh`、`npm run dev` 或需要持续监听端口的命令。
8. 如果依赖已由 Harness 安装完成，可以运行最窄相关的静态检查、单元测试和 build；完整 verify 由外层 Harness 执行。
9. 中文报告：失败原因、修改内容、运行过的命令、剩余风险。
